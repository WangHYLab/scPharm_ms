...
SuppleFigure5a <- function (sample) {
  ...
  for (i in 1:14) {
    ...
    CairoPDF(paste0("./Figure/supplefig5a_GDC0810_", names(data.1)[i],"_T_20231117.pdf"), width = 2, height = 1.4)
...
SuppleFigure5b <- function (sample) {
  ...
  for (i in 1:14) {
    ...
    colnames(meta.data) = c("Group", "NES")
    # Create a PDF file using the CairoPDF function, specifying filename, width, and height
    CairoPDF(paste0("./Figure/supplefig5b_GDC0810_", names(data.1)[i],"_A_20231117.pdf"), width = 2, height = 1.4)
    # Set graphic parameters including outer margins, inner margins, and x-axis tick label positioning
    par(oma = c(1.2, 3, 0.2, 1), mar = c(1, 0, 1.4, 0), xpd = TRUE)
    plot(density(null$NES), col = "#999999", main = "",
        lwd = 1.5, lty = 2, cex.axis = 1.2, font.main = 1, tcl = -0.25, ylab = "", xlab = "",
...
SuppleFigure5c <- function (sample) {
  ...
  CairoPDF("./Figure/supfig5c.pdf", width = 9.8, height = 3.6)
  print(supfig5c)
  dev.off()
}
SuupleFigure5c(sample)


# SuppleFigure 5d ---------------------------------------------------------------

SuppleFigure5d <- function (sample) {
  # Load the drug prioritization of ER positive BRCA samples from the specified path
  data.2 = lapply(sample, function(sam) {
    rank = read.csv(paste("./scPharm/result/", sam, "_scPharm_drug_rank.csv", sep = ""))
    return(rank)
  ...
  for (sam in names(data.2)) {
    ...
    CairoPDF(paste0("./Figure/supfig5d_", sam, "_1113.pdf"), width = 2.2, height = 5.8)
    print(p)
    dev.off()
  }
}
SuppleFigure5d(sample)
