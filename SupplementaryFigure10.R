...
names(sample) = sample
data = list()
for (batch in c('first','second','third','fourth','fifth')) {
  temp = lapply(sample, function(sam){
    rds = readRDS(paste('./result/random_', batch, '/', sam,'_scPharm_metadata_200.rds', sep = ''))
    rds = rds[,which(colnames(rds) %in% grep("scPharm_nes",
                                             colnames(rds),
                                             value = T))]
    return(rds)
  })
...
data = tidyr::gather(data, key = "drug", value = "NES")
CairoPDF("./random_log/density.pdf", width = 5, height = 3)
par(oma = c(1.2, 3, 0.2, 1), mar = c(1, 0, 1.4, 0), xpd = TRUE)
plot(density(data$NES), col = "#999999", main = "",
     lwd = 1.5, cex.axis = 1.2, font.main = 1, tcl = -0.25, ylab = "", xlab = "",
     bty = "l", las = 1, ylim = c(0, 2), xlim = c(-3,3))
abline(v = -1.751302, col = "#FF0000", lty = 2, lwd=1.5)
abline(v = 1.518551, col = "#0000FF", lty = 2, lwd=1.5)
dev.off()
